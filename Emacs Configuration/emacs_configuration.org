...
*** IN-PROGRESS notifications for agenda entries
    ...
    It is important for me that when I work with the org-agenda that I
    get notifications for when things expire for this I use this
    package: [[https://github.com/akhramov/org-wild-notifier.el][org-wild-notifier]].
...
;;   :ensure t)
    ...
    So org-wild notifier did not get it to run properly so far. 
    ...
    Ok so alert seemed to be fucked up for macos - see [[https://github.com/jwiegley/alert/issues/47][this thread]]. 
	 ...
	 (dolist (dl (org-alert--strip-states active))
	   (shell-command-to-string (concatenate 'string "echo " dl " | terminal-notifier -sound default"))))
    ...
    do not want to spend too much time on it. this is also an
    alternative, you should [[https://github.com/vjeantet/alerter][install alerter]] an put it in .emacs.d then:

#+begin_src emacs-lisp 
;; add the path to .emacs.d where you installed alerter
(setq exec-path (append exec-path '("/Users/marcohassan/.emacs.d/")))
#+end_src 

#+RESULTS:
| /usr/local/Cellar/emacs-head@27/HEAD-dcc00bb_1/bin/ | /usr/local/opt/node@10/bin/ | /Users/marcohassan/Desktop/Program_Executables/OpenshiftCLI/ | /usr/local/texlive/2019/bin/x86_64-darwin/ | /usr/local/bin/ | /usr/local/sbin/ | /usr/local/opt/llvm/bin/ | /usr/local/bin/ | /usr/bin/ | /bin/ | /usr/sbin/ | /sbin/ | /Library/TeX/texbin/ | /Users/marcohassan/.go/bin/ | /usr/local/opt/go/libexec/bin/ | /usr/local/Cellar/emacs-head@27/HEAD-dcc00bb_1/libexec/emacs/27.1.91/x86_64-apple-darwin19.0.0/ | /Users/marcohassan/.emacs.d/ | /Users/marcohassan/.emacs.d/ | /Users/marcohassan/.emacs.d/ |

#+begin_src emacs-lisp 
;;; mmmmm not sure it is working.. keep it 1 or two days... if it does
;;; not work cancel it.
;;; can theoreticall also work with termminal-notifier-command if it works.
...
(setq
  appt-message-warning-time '50  ;; send first warning 15 minutes before appointment
